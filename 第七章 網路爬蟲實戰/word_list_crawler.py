...
def generate_urls(url, start_page, end_page): #整理html
    urls = []
    for page in range(start_page, end_page+1):
        urls.append(url.format(page))
    return urls

def get_resource(url): #抓取資料
...
def save_to_csv(words, file): #儲存csv檔
    with open(file, "w+", newline="", encoding="utf-8") as fp:
        writer = csv.writer(fp)
        for word in words:
            writer.writerow(word)

def web_scraping_bot(urls): #主函式
    eng_words = []
    
    for url in urls:
        file = url.split("/")[-1]
        print("抓取: " + file + " 網路資料中...")
        r = get_resource(url) #抓取url
...
if __name__ == "__main__": #主程式
    urls = generate_urls(URL, 1, 9) #generate_urls(url,起始頁數,最終頁數)
    print(urls) 
